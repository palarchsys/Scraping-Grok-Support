...
from sscraping.crawler.spiders.rss import RssSpider
from sscraping.crawler.spiders.site import SiteSpider
from sscraping.scrape.sources import load_sources
...
def scrapy_settings(app: Settings) -> ScrapySettings:
    st = ScrapySettings()
    st.setmodule("sscraping.crawler.settings")
...
def run_crawl(app: Settings, source_id: str | None = None) -> int:
    ...
    for crawler in getattr(process, "crawlers", []):
        ...
        ups = crawler.stats.get_value("sscraping/pg_upserts") or 0
