"""Logs verbeux : console + fichiers rotatifs (sscraping / scrapy / sql / nlp)."""
...
def setup_logging(log_dir: Path, *, quiet: bool = False, level: str = "DEBUG") -> None:
    ...
    root.addHandler(_file("sscraping.log"))

    sql_h = _file("sql.log")
    sql = logging.getLogger("sscraping.sql")
    sql.addHandler(sql_h)
    sql.setLevel(logging.DEBUG)
    sql.propagate = True

    nlp_h = _file("nlp.log")
    nlp = logging.getLogger("sscraping.nlp")
    nlp.addHandler(nlp_h)
    nlp.setLevel(logging.DEBUG)
    nlp.propagate = True

    scrapy_h = _file("scrapy.log")
    for name in ("scrapy", "sscraping.crawler"):
